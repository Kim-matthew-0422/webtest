<?php
	...
	* ZmSpamFree(zmCaptcha) Main Program ver. 1.0
	* 2009-03-01 Released.
	* System requirements : PHP 4.0.6+ / GD Library
	* @section intro 소개
...
if ( isset($_GET['zsfimg']) ) {
	...
	if ( defined('zsfSessId') && is_file( zsfAr.'Log/Connect/'.zsfSessId.'.php' ) && !isset($_GET['re']) && !isset($_GET['cfg']) ) {
		include zsfAr.'Log/Connect/'.zsfSessId.'.php';
	}
	else {
		# 경우에 따라 문제 만들기
	...
	function zsfDirChk ( $dir ) {
		...
		elseif ( substr(sprintf('%o', fileperms($dir)),-4) < 707 ) {
			zsfErr('Directory <span style="color: #c00; font-weight: bold">'.$dir.'</span> must be 777 permission.',50);
		}
	}
	...
	/*
		이미지 출력
	*/
	$thisZsfCfg['logoW'] = 32; # 로고 폭(여백 포함)
...
/* 검증 함수 시작 */
{
	function zsfCheck ( $zsfCode, $zsfLog='', $zsfMethod='POST' ) {
		...
		function zsfLog ( $zsfLogFile,  $zsfQ, $zsfA, $zsfCode, $zsfMethod, $zsfLog, $zsfTerm ) {
			$zsfLogFile = zsfAr.'Log/'.$zsfLogFile.'.php';
			$zsfLogDataArr = array ();
			$i = 0;
