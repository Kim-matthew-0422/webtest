<div class="postsArea"><span class="posts">댓글 <?=$comm_total?>개</span></div>
<div id="commentArea">
		...
		<ul class="commentList">
			...
			<? for($i = 0; $i < count($comm); $i++) { ?>
				<? if(in_array($comm[$i]["view_type"], array("admin", "general"))){ ?>
					...
					<!-- 댓글 폼 -->
					<li class="list_new <?if((strlen($comm[$i]["c_dep"])-1)){?>clamplshape<?}?>" style='padding-left:<?echo (strlen($comm[$i]["c_dep"])-1) * 20 ?>px' >
						<!-- 댓글 제목줄 -->
						<ul class="listMenu">
							<!-- 댓글 글쓴이 출력 -->
							...
							<li id="com_li_modify_top_<?=$i?>" style="display:none; float:left; max-width:95%;">
								<? if($Get_Login == FALSE) { ?>
									<label>작성자</label><input id="com_writer_modify_<?=$i?>" name="" type="text" class="text">&nbsp;&nbsp;
									<label>비밀번호</label><input id="com_passwd_modify_<?=$i?>" name="" type="password" class="text">
								<? } ?>
							</li>
							<!-- 댓글 기능 버튼 출력 -->
							<li class="option" id="iconBox_options_<?=$i?>">
								...
								<? if($comm[$i]["combest"] == TRUE) { ?>
										<span class="best">
											<a href="<?=$comm[$i]["combest"]?>">추천[<?=$comm[$i]["c_best"]?>]<!--img src="/images/common/icon/icon_u.gif" alt="추천"/--></a>
										</span>
								<? } ?>
								<!-- 댓글 기능 : 답변 -->
								<? if($comm[$i]["comreply"] == TRUE) { ?>
										<span>
											<img src="/images/common/icon/icon_re.gif" class="cursor" alt="답변" onclick="com_function_toggle('reply', <?=$i?>);" />
										</span>
								<? } ?>
								<!-- 댓글 기능 : 수정 -->
								<? if($comm[$i]["comedit"] == TRUE) { ?>
										<span>
											<img src="/images/common/icon/icon_m.gif" class="cursor" alt="수정" onclick="com_function_toggle('modify', <?=$i?>);" />
										</span>
								<? } ?>
								<!-- 댓글 기능 : 삭제 -->
							...
							<li class="option" id="iconBox_cencel_<?=$i?>" style='display:none'>
								...
								<span id="com_cencel_<?=$i?>">
									<img src="/images/common/icon/icon_u.gif" class="cursor layerPop02_comdel" alt="실행취소" onclick="com_function_toggle('cencel', <?=$i?>);"/>
								</span>
							</li>
						</ul>
						<!-- 댓글 내용 출력 -->
						...
						<div id="com_li_delete_<?=$i?>" style="display:none;">
							...
							<input type="button" class="btn_black" value="확인" onclick="confirmDel(<?=$i?>);">
						</div>
					</li>
					...
					<li id="com_li_modify_bottom_<?=$i?>" class="cWrite" style="display:none;">
						...
						<table cellpadding="0" cellspacing="0" width="100%">
							...
							<tr>
								...
								<td class="confirm">
									<input type="button" class="btn_comment" value="등록" onclick="blank_check('COMMODIFY', 'modify', <?=$i?>);">
								</td>
							</tr>
						</table>      
					</li>
					...
					<li id="com_li_reply_<?=$i?>" class="cWrite" style="display:none; padding-left:<?echo (strlen($comm[$i]["c_dep"])) * 15 ?>px">
						<!-- 답변 제목줄 -->
						<div class="userInfo">
								<label>작성자</label><input id="com_writer_reply_<?=$i?>" name="" type="text" class="text" value="<?=$sess["username"]?>">&nbsp;&nbsp;
							<? if($Get_Login == FALSE) { ?>
								<label>비밀번호</label><input id="com_passwd_reply_<?=$i?>" name="" type="password" class="text">
							<? } ?>
						</div>
						<!-- 답변 내용 -->
						<table cellpadding="0" cellspacing="0" width="100%">
							...
							<tr>
								...
								<td class="confirm">
									<input type="button" class="btn_comment" value="등록" onclick="setModeAndParam('COMREPLY', 'reply',<?=$i?>);">
								</td>
							</tr>
						</table>      
					</li>
			...
			<? if($sess["userlevel"] >= $Board_Admin["level_com"]) { ?>
				<li id="com_li_write" class="cWrite">
					<!-- 댓글 제목줄 -->
							<div class="userInfo">
									<label>작성자</label><input id="com_writer_write_<?=$i?>" name="" type="text" class="text" value="<?=$sess["username"]?>">&nbsp;&nbsp;
								<? if($Get_Login == FALSE) { ?>
									<label>비밀번호</label><input id="com_passwd_write_<?=$i?>" name="" type="password" class="text">
								<? } ?>
							</div>
					<!-- 댓글 내용 -->
					<table cellpadding="0" cellspacing="0" width="100%">
						...
						<tr>
							...
							<td class="confirm">
								<input type="button" class="btn_comment" value="등록" onclick="blank_check('COMFORM', 'write', <?=$i?>);">
							</td>
						</tr>
					</table>      
				</li>
...
function confirmDel(index){
	...
	if(confirm("삭제하시겠습니까?<? echo ($sess['userlevel'] >= 100) ? '\n\n(최고관리자 권한으로 삭제시 복구할 수 없습니다.)' : '';?>")) {
