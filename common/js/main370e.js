...
(function (root, factory) {
    ...
    $.extend(Plugin.prototype, {
        ...
        init: function () {
            var thisBgSrc;
            if (this.image) {
                thisBgSrc = this.image;
                this.$scrollingElement = $('<div></div>').addClass(this.settings.imgClass);
            } else {
...
(function ($, document) {
    ...
    $(document).ready(function() {
        ...
        init.instafeed.init();
        init.matchHeights();
        init.photoswipe.init();
        init.sticky.init();
        init.selectric.init();
        init.selectric.pageNav.init();

        // App
    	// app.matchScroll.init();
        app.accordion.init();
        app.banner.init();
        app.header.init();
        app.progress.init();
		...
		app.video.init();
		app.notifyBar.init();
		app.fullscreenImageHero.init();
        // Pages
		pages.stockists.init();
		if ($('body').hasClass('home')){ pages.home.init(); }

        // Story
        story.footnote.init();
        story.progressPosition.init();
        story.publishIssuePosition.init();
		story.paywall.init();
		story.pagination.init();
    ...
    var app = {
            ...
            _open : function() {
                ...
                $('.c-Accordion__title').on('click', function(e) {
                    ...
                    if( $(this).next().is(':hidden') ) {
                        self._close();
                        $(this).addClass('c-Accordion--active');
                        $(this).next().slideDown();
                    } else {
        ...
        banner : {
            ...
            get_parallax_banners : function() {
                ...
                if( Foundation.MediaQuery.atLeast('xlarge') ) {
                    ...
                    $('.m-advert--parallax').each(function(i) {

                        var $this = $(this).find('.m-advert__image').not('.show-for-small-only');

                        var bgSrc = $this.find('img')[0].dataset.noscript,
                            height =  $this.find('img')[0].height,
                            width =  $this.find('img')[0].width;

                        $this.attr('data-image', bgSrc);
                        $this.attr('width', "100%");
                        $this.attr('height', "480px");
                        $this.find('img').remove();

                        $('.m-advert-').imagesLoaded(function() {
                            $this.imageScroll({
                                // coverRatio: 0.3
                                touch: false,
                                holderMinHeight: 480,
            ...
            header : function() {
                ...
                if( $('.m-advert--fixed').length > 0 ) {

                    var $banner_fixed = $('.m-advert--fixed'),
                        banner_height = $banner_fixed.outerHeight( true );

                    $banner_fixed.next('.m-advert__body-container').css('margin-top', banner_height);
        ...
        header : {
            init : function() {
                ...
                this.story_fixed.init();
                this.global_fixed.init();
            ...
            story_fixed : {
                init : function() {
                    ...
                    if( Foundation.MediaQuery.atLeast('xlarge') && $('.l-Header--fixed').length > 0 ) {
                        self.onScroll();
                        ...
                        // Reset Settings
                        $('.l-Header--fixed').removeClass('l-Header--fixed--active');
                    }
                ...
                scrollHandler : function() {
                    ...
                    var offsetTop = 0,
						$headerFixed = $('.l-Header--fixed');


                    if( ($(window).scrollTop() > offsetTop) && (document.body.getBoundingClientRect().top < scrollPos) ) {

						moreStories.loadStories();
						$headerFixed.addClass('l-Header--fixed--active');

                    } else {
						$headerFixed.removeClass('l-Header--fixed--active');
					}
            ...
            global_fixed : {
                init : function() {
                    ...
                    var self = this;
                        $elOffset = $('.l-Header'),
                        offsetTop = $elOffset.offset().top,
                        offsetBottom = offsetTop + $elOffset.outerHeight(true);

                    // Global Header not Single Post or Homepage
                    if( /*Foundation.MediaQuery.atLeast('medium') &&*/ !$('.single-post').length > 0 && !$('.home.page').length > 0 ) {

                        self.onScroll( offsetBottom );

                    // Homepage Header
                    } else if( /*Foundation.MediaQuery.atLeast('medium') &&*/ $('.home.page').length > 0 ) {

                        // Check if top advert
                        // if( $('body > .m-advert').length > 0 ) {
                        if( $('body > .m-advert--full').length > 0 ) {

                            var advertHeight;

                            // $('body > .m-advert').each(function() {
                            $('body > .m-advert--full').each(function() {
                                    advertHeight = advertHeight + $(this).outerHeight(true);
                            });

                            advertHeight = advertHeight + $('.l-Header').outerHeight(true);

                        }


                        if( $('body').scrollTop() >= advertHeight || typeof advertHeight === 'undefined' ) {
                            $('.l-Header').css('position', 'fixed');
                            self.onScroll( offsetBottom );
                        } else {

                            // Set to absolute which will attach to the relative parent
                            $('.l-Header').css('position', 'absolute');

                            // Check once relative parent hits scrollTop and then apply normal fixed position
                            var triggerScroll = 0;

                            $(window).on('scroll', function() {

                                if( $('.m-advert__body-container').offset().top <= $(window).scrollTop() ) {
                                    $('.l-Header').css('position', 'fixed');
                    ...
                    // Anything else other that Single Post as header.story_fixed();
                ...
                scrollHandler : function( e ) {
                    ...
                    // Start header as sticky
                    if( $(window).scrollTop() > e.data.offset ) {
                        $header.addClass('l-Header--sticky');
                    } else {
                        $header.removeClass('l-Header--sticky');
                    }

                    // Sticky header active
                    if (scrollTop >= e.data.lastScroll){
                        // Going Down
                        $header.removeClass('l-Header--sticky--active');
                    } else {
                        // Going Up
                        $header.addClass('l-Header--sticky--active');
                    }
                    e.data.lastScroll = scrollTop;

                },

                onScroll : function( offsetBottom ) {

                    var self = this;

                   // $(window).on('scroll', {'offset':offsetBottom, 'lastScroll' : 0}, self.scrollHandler );
        ...
        loadMore : {
            ...
            init : function() {
                ...
                $('.js-repeater-load-more').on('click', function(e) {

                    e.preventDefault();

                    var $btn = $(this);
                    var $container = $btn.closest('.module');
                    var items = $container.find('.item-container').filter(":hidden:lt(6)");

                    items.show();

                    if( $container.find('.item-container').filter(":hidden").length == 0 ) {
                        $btn.remove();
                    }
        ...
        matchScroll : {
            ...
            init : function() {
                ...
                $('.js-matchScroll-right').matchScroll({
                    target: $('.js-matchScroll-left'),
                    offset: 50
                });
        ...
        progress : {
            ...
            init : function () {
                ...
                // Check images are loaded to get true body height
            ...
            scroll : function( height ) {
                ...
                var scrollHandler = function() {
                    ...
                    var percentage = $(window).scrollTop() / height;

                    self.update( percentage.toFixed(3) );
            ...
            update : function( percentage ) {
                ...
                if( percentage >= 1 ) {
                    // Force full width
                    $('.c-Progress__line').css({width : '100%'});
                } else {
                    $('.c-Progress__line').css({width : percentage * 100 + '%'});
                }
        ...
        overlay : {
            ...
            trigger : function() {
                ...
                $('.js-overlay-trigger').on('click', function(e) {
					e.preventDefault();
                    if( $('.c-overlay').hasClass('c-overlay--active') ) {
            ...
            focus : function() {
                ...
                $('.c-overlay input[type="text"]').select();
		...
		notifyBar: {
			...
			init: function () {
				...
				if ($('.c-NotifyBar').length > 0) {
					$('body').addClass('has-c-NotifyBar');
				}
		...
		fullscreenImageHero: {
			...
			init: function () {
				if ($('.c-FullScreenImageHero').length > 0) {
					const that = this;

					$('.c-FullScreenImageHero').each(function (index) {
						const componentHeight = $(this).height();
						const window_height = $(window).height();

						if(componentHeight > window_height) {
							that.calculateTextOffset(this, window_height);
							$('.c-FullScreenImageHero__text__main', this).css('position', '').css('top', '');
						} else {
							//console.log('too small');
							$('.c-FullScreenImageHero__text__main', this).css('position', 'absolute').css('top', '50%');
							$('.c-FullScreenImageHero__text', this).css('padding-top', '');
						}
			...
			calculateTextOffset: function (el, window_height) {
				const txt_box_height = $('.c-FullScreenImageHero__text__main', el).height();
				let header_height = $('.l-main').css('margin-top');
				header_height = parseInt(header_height, 10); // make unitless
				header_height = header_height + $('.l-Header').height();
				//console.log(txt_box_height);
				//console.log(header_height);

				let offset = window_height - txt_box_height;
				offset = offset - header_height + 'px';

				$('.c-FullScreenImageHero__text', el).css('padding-top', offset);
        ...
        video : {
            ...
            vimeo : {
                ...
                iframe : ( $('.m-video--vimeo').length > 0 ) ? $('.m-video--vimeo iframe')[0] : false,
                player : false,
                ...
                autoplay : function() {
                    ...
                    if( self.iframe ) {
                        ...
                        self.player = new Vimeo.Player( app.video.vimeo.iframe )
                ...
                play : function() {
                    ...
                    $('.m-video--vimeo .js-play').on('click', function() {

                        app.video.remove_overlay( $(this) );

                        self.player.setCurrentTime(0).then(function() {
            ...
            embed : {
                ...
                play : function() {
                    ...
                    $('.m-video--embed .js-play').on('click', function() {

                        var $iframe = $(this).parents('.m-video--embed').find('iframe');
            ...
            remove_overlay : function( $this ) {

                $this.parents('.m-video__content').fadeOut();
    ...
    var shop = {
        ...
        bundled : {
            ...
            syncCheckbox : function() {
                ...
                $('.bundled_product_checkbox').change(function() {
                    $(this).parent().prev('input').prop("checked", this.checked);
                });
        ...
        cart : {
            ...
            cartText : $('.single_add_to_cart_button').text(),
            ...
            init : function(){
                ...
                // Disable Add to Cart once a product has been added
                $('.single_add_to_cart_button').on('click', function() {
                    shop.cart.button.loading();
                });
                ...
                $('body').on('added_to_cart',function(){
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if( data != '' ) {
                                // Update cart count
                                $('.l-Header__cart span').text('(' + data + ')');
                                $('.l-Header__cart').removeClass('hidden');
                            } else {
                                // Nothing returned
                    ...
                    $.ajax({
						...
						success: function (data) {
                            ...
                            if( data != '' ) {
                                ...
                                shop.cart.button.reset();

                                // Update mini cart
                                $('.c-MiniCart__wrapper').empty().append(data);

                                // Show header and basket
                                if( $('.l-Header--fixed').length > 0 ) {
                                    app.header.story_fixed.hideHeader();

                                    setTimeout(function() {
                                        // Open mini cart
                                        that.openMiniCart();
                                    }, 350);
                                } else if( $('.l-Header').length > 0 ) {
                                    app.header.global_fixed.showHeader();
                ...
                $('.l-Header__cart').on('click', function(e){
                    e.preventDefault();

                    if( $('.c-MiniCart').is(':visible') ) {
                        that.closeMiniCart();
                    } else {
                        that.openMiniCart();
                    }
                ...
                $(window).on('scroll', that.closeMiniCart );
            ...
            button : {
                ...
                reset : function() {
                    ...
                    $('.single_add_to_cart_button').text( shop.cart.cartText );

                },

                loading : function() {
                    $(document).find('.single_add_to_cart_button').text('Adding...');
            ...
            openMiniCart: function(){
                ...
                var $miniCart = $('.c-MiniCart');

                if( $miniCart.is(':hidden') ) {

                    var headerOffset = $('.l-Header').offset().top + 100 - $(document).scrollTop();

                    $miniCart.css('top', headerOffset);

                    $miniCart.stop().fadeIn(250, function() {
                        // Close on page click
                        $(document).on('click', function() {
                            shop.cart.closeMiniCart();
                        });
                    }).on('click', function(e) {
                        e.stopPropagation();
                    });
        ...
        wholesale : {
            ...
            init : function() {
                if($('#showwholesale').length)
                {
                    $('#wholesale-wrapper').hide();
                }
                var that = this;
                $('#showwholesale').on('change',function(){
                    that.toggleWholesale($(this));
                });

            },
            toggleWholesale : function($el) {
                if($el.is(':checked'))
                {
                    $('#wholesale-wrapper').show();
                }
                else
                {
                    $('#wholesale-wrapper').hide();
    ...
    var pages = {
        ...
        stockists : {
            ...
            placeholder : function() {
                ...
                $('#addressInput').attr('placeholder', 'enter city or postcode');
            ...
            reCheckHeights : function() {
                ...
                $(document).ajaxComplete(function() {
                    ...
                    $('.slp_map__container').css('height', ($('#map_sidebar').height()) + $('#searchForm').height() );
		...
		home : {
			...
			logoCheck: function () {
				...
				$('.l-Header__logo img').css('transform', '');
				if (Foundation.MediaQuery.atLeast('large')) {

					const logo_matrix_transforms = {
						'large': [3.5, 0, 0, 3.5, 0, 112],
						'xlarge': [4.5, 0, 0, 4.5, 0, 170],
						'xxlarge': [5, 0, 0, 5, 0, 190],
						'xxxlarge': [6, 0, 0, 7.5, 0, 214.5],
						'max': [7.5, 0, 0, 7.5, 0, 214.5]
					}
					let transform = logo_matrix_transforms[Foundation.MediaQuery.current];

					//let transform = pages.home.getMatrixAsArray($('.l-Header__logo').css('transform'));
					let originalScale = transform[0];

					//console.log('originalScale = ' + originalScale);

					let originaltranslateY = transform[5];
					//console.log('originaltranslateY = ' + originaltranslateY);
					...
					if ($('.l-Header__logo').css('opacity') == '0') {
						setTimeout(function(){ $('.l-Header__logo').css('opacity', '1'); }, 2000);
					}
			...
			logoAnimation: function (originalScale, originalTransformY) {
				...
				const mod = .835;// makes the scroll percentage end at 1 instead of 0... magic number!
				...
				// if (translateY > originalTransformY) {
				// 	translateY = originalTransformY;
				// }
				//$('.l-Header__logo').css('transform', 'scale(' + scale + ')');
				//$('.l-Header__logo').css('transform', 'scale(' + scale + ') translateY(' + translateY + 'px)');

				$('.l-Header__logo img').css('transform', 'matrix(' + scale + ', 0, 0, ' + scale + ', 0, ' + translateY + ')');
			...
			getHeaderHeight() {
				...
				const header_height = $('.l-Header').height();
    ...
    var story = {
        ...
        paywall : {
            init : function() {
                //trigger user selection of paywall article
                var that = this;
                var clicked = false;
                $('#paywall-click').on('click', function(e){
                    ////console.log('Paywall Click');
                    e.preventDefault();
                    clicked = true;
                    //set cookie
                    var d = new Date();
                    ...
                    var cookie_item_position = that.getCookie('kfp01'+$('#shdhfsi').val()+'4113230131');
                    if(cookie_item_position < 3)
                    {
                        var name = 'kfp01'+$('#shdhfsi').val()+'4113230131['+cookie_item_position+']';
                        var value = $('#kiyne').val();
                ...
                $('#paywall-form').on('submit',function(e){
                    if(!clicked)
                    {
                        e.preventDefault();
                    }
            ...
            getCookie : function(name)
            {
                ...
                var count = (document.cookie.match(re) || []).length;
                return count;
            }
        },
        ...
        footnote : {
            ...
            init : function() {
                ...
                if( $('.c-Footnote').length > 0 && $('.c-gallery').length > 0 && Foundation.MediaQuery.atLeast('xlarge') ) {

                    // Check once first LQIP image is loaded
                    $('.c-gallery__image:first-of-type()').imagesLoaded(function() {

                        // Check for multiple foot notes on one page
                        $('.c-Footnote').each(function() {

                            var $this = $(this);

                            // Check for second image to anchor too
                            if( $this.parents('.row').find('.c-gallery__image').length > 1 ) {

                                var mt = self.getImageHeight( $this.parents('.row').find('.c-gallery') ) - self.getCreditHeight( $this.parents('.column') );

                                // Dont allow to be a minus number
                                if( mt >= 0 ) {
                                    $this.css('margin-top', mt );

                                    // Update publish position
                                    // story.publishIssuePosition.init();
                                }
            ...
            getImageHeight : function( $this ) {

                return $this.find('figure:first-of-type').outerHeight( true );

            },

            getCreditHeight : function( $this ) {

                return $this.find('.c-credits').outerHeight( true );
        ...
        publishIssuePosition : {
            ...
            init : function() {
                ...
                // if( $('.c-Footnote').length > 0 ) {
                //     $('.c-Footnote').imagesLoaded( function() {
                //         $('.js-publish-issue').css('height', 'calc(100% - ' + self.getContentsHeight() + 'px)');
                //     });
                // } else {
                //     $('.js-publish-issue').css('height', 'calc(100% - ' + self.getContentsHeight() + 'px)');
                // }
            ...
            getContentsHeight : function() {
                ...
                var $parent = $('.js-publish-issue').parents('.match'),
                    childrenHeight = 0;

                $( $parent ).children().each(function() {

                    if( !$(this).hasClass('js-publish-issue') ) {
        ...
        progressPosition : {
            ...
            onScroll : function() {
                ...
                var $progress = $('.c-Progress__container');

                $(window).on('scroll', function() {
                    // //console.log('scrolling')
                    // if( $(window).scrollTop() > ( $('.l-Header').outerHeight(true) - 1 ) ) {
                    //     $progress.addClass('c-Progress--fixed');
                    // } else {
                    //     $progress.removeClass('c-Progress--fixed');
                    // }
		...
		pagination: {
			// hides the next/prev posts on single articles when the new hero components are in use
			init: function () {

				const heros = ['.c-FullScreenImageHero', '.c-AlignedImageHero', '.c-BigImageHero', '.c-SplitImageHero']

				heros.forEach(hero => {

					// if not using one of the new heros exit
					if ($(hero).length < 1) {return; }

					$('.c-pagination').addClass('js-inactive');

					$(window).on('scroll', function() {
						waitForFinalEvent(function() {

							if( $(window).scrollTop() > ( $(hero).outerHeight(true) ) / 1.5) {
								$('.c-pagination').removeClass('js-inactive');
							} else {
								$('.c-pagination').addClass('js-inactive');
							}
						}, 400, 'pagination-throttle');
					});
    ...
    var slideshow = {
        ...
        multi : function() {
            ...
            var $slideshow = $('.m-slideshow--multi');

            if( $slideshow.length > 0 ) {

                // Slider next and prev

                $(document).on('click', '.slick-list__overlay--prev', function() {
                    $slideshow.slick('slickPrev');
                });

                $(document).on('click', '.slick-list__overlay--next', function() {
                    $slideshow.slick('slickNext');
                });

                // Add fake arrow areas to DOM

                $slideshow.on('init', function() {

                    $(this).find('.slick-list').before('<span class="slick-list__overlay slick-list__overlay--prev"></span>');
                    $(this).find('.slick-list').after('<span class="slick-list__overlay slick-list__overlay--next"></span>');
                });

                // Disable next and prev if no more slides
                $slideshow.on('init afterChange', function( slick, currentSlide ) {

                    $('.slick-list__overlay--disabled').removeClass('slick-list__overlay--disabled');

                    if( currentSlide.currentLeft === 0 ) {
                        // //console.log('We are at the End');
                        $('.slick-list__overlay--next').addClass('slick-list__overlay--disabled');
                    }

                    if( currentSlide.currentSlide === 0 ) {
                        // //console.log('We are at the Start');
                        $('.slick-list__overlay--prev').addClass('slick-list__overlay--disabled');
                    }

                })
				let dots;
				if ($slideshow.hasClass('m-slideshow--multi--no-dots')) {
					dots = false;
				} else {
					dots = true;
				}
		...
		centred : function() {
			...
			var $slideshow = $('.m-slideshow--centred');
            if( $slideshow.length > 0 ) {
        ...
        single : function() {
            ...
            var $slideshow = $('.m-slideshow--single'),
                i = 0,
                shallowest;
            ...
            if( $slideshow.length > 0 ) {
                ...
                //     (function setHeight() {
                //         $slideshow.on('init setPosition', function() {
                ...
                //                     // Set first one.
                //                     if( i === 0 ) {
                //                         shallowest = $this.height();
                //                     }
                //                     shallowest = ( $this.height() < shallowest ) ? $this.height() : shallowest;
                //                 }

                //                 i++;

                //             	$('.single-product .m-slideshow--single img').css('max-height', shallowest);
                //             });
                ...
                //             $('.m-slideshow__wrapper > .c-gallery__expand').data('index', currentSlide.currentSlide);
                //         });
                ...
                $slideshow.slick({
                    ...
                    arrows: true,
                    fade: true,
                    autoplay: ( $('.single-product').length > 0 ) ? false : true,
                    autoplaySpeed: 5000,
                    responsive : [
                        {
                            breakpoint : breakpoint.xlarge,
                ...
                if( $('.single-product').length > 0 && Foundation.MediaQuery.atLeast('xlarge') ) {
        ...
        singleWide : function() {
            ...
            var $slideshow = $('.m-slider-wide'),
                $slideshow_fade = $('.m-slider-wide--fade');
            ...
            if( $slideshow.length > 0 ) {
                ...
                $slideshow_fade.on('init afterChange', function() {
                    $slideshow_fade.find('.slick-active--fade').removeClass('slick-active--fade');

                    setTimeout(function() {
                        $slideshow_fade.find('.slick-active').addClass('slick-active--fade');
                    }, 100);

                });

                $slideshow_fade.slick({
                    slidesToShow: 1,
                    slidesToScroll: 1,
                    arrows: false,
                    fade: true,
        ...
        story : function() {
            ...
            var $slideshow = $('.c-gallery--slideshow');

            if( $slideshow.length > 0 ) {

                $slideshow.on('init breakpoint', function() {
                ...
                $slideshow.each(function() {
                    ...
                    var $this = $(this);
                    if( $this.find('.c-gallery__image').length <= 1 ) {
                        $this.slick('slickSetOption', 'variableWidth', false, true);
                        $this.addClass('c-gallery--slideshow--single');
                        $this.parents('.column').addClass('medium-9');
                    }
                });
    ...
    var mailchimp = {
        ...
        headerForm : function() {
            ...
            $('.js-menu-newsletter').on('click', function(e) {
        ...
        callback : function( $this, message, success ) {

            var successClass = ( success === true ) ? 'c-newsletter__notification--success' : 'c-newsletter__notification--error';

            if( $this.find('.c-newsletter__notification').length > 0 ) {
                if( success === true ) {
                    $this.find('.c-newsletter__notification').removeClass('c-newsletter__notification--error');
                    $this.find('.c-newsletter__notification').addClass( successClass );
                }
                $this.find('.c-newsletter__notification').text( message );

                if( !$this.find('.c-newsletter__notification').is(':visible') ) {
                    $this.find('.c-newsletter__notification').slideDown();
                }
            } else {
                $this.append('<div class="c-newsletter__notification ' + successClass + '" style="display: none;">' + message + '</div>');
                $this.find('.c-newsletter__notification').slideDown();
            }
            ...
            if( success === true ) {
                ...
                setTimeout(function() {
                    $this.find('.c-newsletter__notification').slideUp();
                }, 3000);
            }
        ...
        submit : function() {
            ...
            $('.c-newsletter').submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if( data == 'success' ) {
                            ...
                            mailchimp.callback( $this, 'Thank you!', true );

                        } else if( (data.indexOf('domain') !== -1) || (data.indexOf('must') !== -1) ) {

                            mailchimp.callback( $this, 'You have entered an invalid email address. Please try again.', false );

                        } else {

                            mailchimp.callback( $this, data, false );
    ...
    var init = {
        ...
        photoswipe : {
            ...
            init : function() {
                ...
                $('.m-slideshow__wrapper .c-gallery__expand, .c-gallery .c-gallery__expand').on('click', function(e) {
                    e.preventDefault();

                    self.initPswp( $(this).data('index') );
        ...
        pushy : function() {
            ...
            function togglePushy( dir ){
                //add class to body based on menu position
                if( dir == 'left' || body.hasClass(pushyOpenLeft) ){
                    body.toggleClass(pushyOpenLeft);
                }else if( dir == 'right' || body.hasClass(pushyOpenRight) ){
        ...
        selectric : {
            ...
            pageNav : {
                ...
                setUp : function() {
                    ...
                    $('<select />').appendTo('.page__nav__wrapper');

                    // Create default option "Go to..."
                    $('<option />', {
                        "selected": "selected",
                        "value"   : "",
                        "text"    : ( $('.woocommerce').length > 0 ) ? ( $('.page__nav__item .active').length > 0 ? $( $('.page__nav__item .active')[0] ).text() : 'Shop:' ) : $('.page__nav__item--active a').text()
                    }).appendTo('.page__nav__wrapper select');

                    // Populate dropdown with menu items
                    $('.page__nav__item a').each(function() {
                ...
                fireSelectric : function() {
                    ...
                    $('.page__nav__wrapper select').selectric({
                        disableOnMobile: false,
                        nativeOnMobile: false,
                        onInit: function() {
                            $('.page__nav__wrapper .selectric').addClass('selectric--alt');
                        }
                    });
                ...
                navChange : function() {
                    ...
                    $('.page__nav__wrapper select').on('change', function() {
        ...
        sticky : {
            ...
            init: function() {
                ...
                var self = this;
                    // count = self.storyCheckHeights();

                // if( self.storyCheckHeights() && Foundation.MediaQuery.atLeast('xlarge') ) {
                //     self.story.init( count );
                // }

                if( $('.slp_map').length > 0 && Foundation.MediaQuery.atLeast('xlarge') ) {
                    self.stockists.init();
                }
            ...
            stockists : {
                ...
                resize : function() {
                    ...
                    if( !Foundation.MediaQuery.atLeast('xlarge') && $('.slp_map__inner').length > 0 ) {

                        if( $('.wrapper-sticky').length > 0 ) {
                            self.$map.hcSticky('destroy');
                            self.$map.removeAttr('style');
                        }

                    } else if ( $('.slp_map__inner').length > 0 ) {
                        self.init();
                    }
                }
            },
            ...
            //             for( i = 1; i <= count; i++ ) {
            //                 $('.sticky--' + i).hcSticky({
            //                     top: 50 + $('.l-Header--fixed').outerHeight(true),
            //                     bottom: 50,
            //                     // offResolutions: - breakpoint.xlarge
            //                     offResolutions: -1200
            //                 });
            ...
            //     $('.single-post__text').each(function() {

            //         var $this = $(this),
            //             textHeight = $this.outerHeight(true),
            //             $gallery = $this.parent().prev().find('.c-gallery--slideshow'),
            //             galleryHeight = $gallery.outerHeight(true);

            //         if( textHeight >= galleryHeight ) {
            //             $gallery.addClass('sticky--' + count);
            //         } else {
            //             $this.addClass('sticky--' + count);
            //         }
            //         count++;
            //     });
    ...
    var homepageModal = {
		init: function () {
			var that = this;
            if ($('.js-home-modal').length && that.getCookie('modalShown') != true) {
				this.show();
				this.triggerClose();
            }
        },
        show: function() {
            ...
            window.addEventListener('scroll', () => {
                if (window.scrollY >= triggerPoint && !triggered) {
                    ...
                    $('.js-home-modal').css('display', 'flex').hide().fadeIn()
					triggered = true;
                }
            })
        },
        close: function() {
            $('.js-home-modal').fadeOut()
            fullscreenOverlay.close()
        },
        triggerClose: function() {
			$('.js-close-home-modal').click(() => {

				//set cookie
				var d = new Date()
				d.setTime(d.getTime() + (30*24*60*60*1000))
				document.cookie = "modalShown=true; expires="+ d.toUTCString() + "; path=/"
	...
	var cookieBanner = {
		...
		init: function () {
			if ($(".c-CookieBanner").length != 0) {
				$('.js-close-cookie-banner').click(function (event) {
		...
		closeBanner: function () {
			$(".c-CookieBanner").addClass('inactive');
			//set cookie
			var d = new Date()
			d.setTime(d.getTime() + (30*24*60*60*1000))
			document.cookie = "hide-cookie-banner=true; expires="+ d.toUTCString() + "; path=/"
		},
    ...
    var burgerMenu = {
        ...
        triggerOpen: function() {
            $('.js-open-burger').click(() => {
                fullscreenOverlay.open()
                $('.js-burger-menu').addClass('is-open')
            })
        },
        close: function() {
        ...
        setTopHeight: function() {
            $('.js-burger-top').height(window.innerHeight - $('.js-burger-actions').outerHeight() - 40)
        }
	}
	...
	var moreStories = {
        init: function() {
			...
			//test
			//$('.js-moreStories-open').click();

        },
        triggerOpen: function() {
            $('.js-moreStories-open').click(() => {
				fullscreenOverlay.open();
				$('.c-MoreStories').addClass('is-open');
            })
        },
        close: function() {
            $('.c-MoreStories').removeClass('is-open')
            fullscreenOverlay.close()
        },
        triggerClose: function() {
            $('.js-close-MoreStories').click(() => {
                moreStories.close()
            })
        },
		loadStories: function () {

			if ($('.c-MoreStories').hasClass('stories-loaded')) { return false; }

			$('.c-MoreStories').addClass('stories-loaded');
            ...
            $.ajax({
				...
				dataType: 'html',
				data: {
					action: 'kf_load_stories',
					stop_category: wpAPIData.storyCategory,
				},
				error: function() {
					console.log('error');
					$('.c-MoreStories').removeClass('stories-loaded');
				},
				success: function (data) {
					if (data !== false) {
						$('.c-MoreStories__stories').html(data);
					} else {
						$('.c-MoreStories').removeClass('stories-loaded');
					}
    ...
    var fullscreenOverlay = {
        ...
        open: function() {
            $('.js-fullscreen-overlay').fadeIn()
        },
        close: function() {
            $('.js-fullscreen-overlay').fadeOut()
        },
        triggerComponentsToClose: function() {
            $('.js-fullscreen-overlay').click(() => {
                if ($('.js-burger-menu').hasClass('is-open')) {
					burgerMenu.close();
					fullscreenOverlay.close();
                }

				if ($('.js-home-modal').is(':visible')) {
					homepageModal.close();
				}

				if ($('.c-MoreStories').hasClass('is-open')) {
					moreStories.close();
				}
    ...
    var newsletterSignup = {
        ...
        enableSubmit: function() {
            $(document).on('change', '.js-newsletter-consent', function() {
                const $this = $(this)
                const $btn = $('.js-newsletter-submit')
    ...
    var singleStory = {
        init: function() {
            this.toggleNextPrevOnScroll()
        },
        resize: function() {
            this.toggleNextPrevOnScroll()
        },
        toggleNextPrevOnScroll: function() {
            if (Foundation.MediaQuery.atLeast('xlarge') && $('.js-pagination-anchor').length) {
                var triggerPoint = $('.js-anchor-pagination-trigger').offset().top - $('.l-Header').outerHeight()
                var triggered = false

                window.addEventListener('scroll', function() {
                    var currentScrollY = window.scrollY

                    if (currentScrollY > triggerPoint && !triggered) {
                        $('.js-pagination-anchor').fadeIn()
                        triggered = true

                    } else if (currentScrollY <= triggerPoint && triggered) {
                        $('.js-pagination-anchor').fadeOut()
                        triggered = false
                    }
                })
            }
